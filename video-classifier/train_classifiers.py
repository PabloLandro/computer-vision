...
def train_mlp_classifier(
    ...
    report_dir = REPORTS_DIR / "mlp"
    prediction_dir = PREDICTIONS_DIR / "mlp"
    ...
    class MultiHeadMLP(nn.Module):
        def __init__(
            ...
            super().__init__()
            self.verb_backbone = self.make_backbone(input_dim, dropout=0.25)
            self.noun_backbone = self.make_backbone(input_dim, dropout=0.40)
            self.verb_head = nn.Linear(768, num_verbs)
            self.noun_head = nn.Linear(768, num_nouns)
            self.action_head = nn.Linear(1536, num_actions)

        @staticmethod
        def make_backbone(input_dim: int, dropout: float) -> nn.Sequential:
            layers = []
            prev_dim = input_dim
            for hidden_dim in (1536, 768):
                layers.extend(
                    [
        ...
        def forward(self, inputs: Any) -> tuple[Any, Any, Any]:
            verb_hidden = self.verb_backbone(inputs)
            noun_hidden = self.noun_backbone(inputs)
            combined_hidden = torch.cat([verb_hidden, noun_hidden], dim=-1)
            return (
                self.verb_head(verb_hidden),
                self.noun_head(noun_hidden),
                self.action_head(combined_hidden),
            )

    def make_loader(indices: Any, shuffle: bool) -> Any:
        dataset = TensorDataset(
            torch.from_numpy(x[indices]).float(),
            torch.from_numpy(y_verb_encoded[indices]).long(),
            torch.from_numpy(y_noun_encoded[indices]).long(),
            torch.from_numpy(y_action_encoded[indices]).long(),
        )
        return DataLoader(
            dataset,
    ...
    model = MultiHeadMLP(
        input_dim=x.shape[1],
        num_verbs=len(encoded_to_verb),
        num_nouns=len(encoded_to_noun),
        num_actions=len(encoded_to_action),
    ).to(device)
    ...
    for epoch in progress(range(1, MLP_EPOCHS + 1), desc="Training MLP epochs"):
        model.train()
        train_loss = 0.0
        train_count = 0
        for batch_x, batch_verb, batch_noun, batch_action in progress(
            train_loader,
            ...
            verb_logits, noun_logits, action_logits = model(batch_x)
        ...
        if epoch_record["val_loss"] < best_val_loss:
            ...
            torch.save(
                {
                    ...
                    "encoded_to_action": {
                        ...
                        for k, (verb_id, noun_id) in encoded_to_action.items()
                    },
                    "config": {
            ...
            stale_epochs += 1
            if stale_epochs >= MLP_EARLY_STOP_PATIENCE:
                break

    checkpoint = torch.load(best_path, map_location=device)
    model.load_state_dict(checkpoint["model_state_dict"])
    write_json(model_dir / "label_maps.json", {
        "encoded_to_verb": {str(k): v for k, v in encoded_to_verb.items()},
        "encoded_to_noun": {str(k): v for k, v in encoded_to_noun.items()},
        "encoded_to_action": {
            str(k): [int(verb_id), int(noun_id)]
            for k, (verb_id, noun_id) in encoded_to_action.items()
        },
    ...
    def predict(indices: Any) -> tuple[Any, Any]:
        loader = make_loader(indices, shuffle=False)
        pred_verbs = []
        pred_nouns = []
        model.eval()
        with torch.no_grad():
    ...
    for split_name, indices in (("val", splits["val"]), ("test", splits["test"])):
        pred_verb, pred_noun = predict(indices)
...
def main() -> None:
    ...
    write_json(REPORTS_DIR / "block" / f"dataset_summary_{run_timestamp}.json", dataset_summary)

    results = [train_mlp_classifier(x, y, splits, verb_map, noun_map, run_timestamp)]

    write_json(
        REPORTS_DIR / "block" / f"all_metrics_{run_timestamp}.json",
        {"run_timestamp": run_timestamp, "results": results},
    )
    print(f"Finished. Reports: {REPORTS_DIR}. Predictions: {PREDICTIONS_DIR}.")
