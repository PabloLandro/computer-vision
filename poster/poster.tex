...
\begin{column}{0.22\paperwidth}
  ...
  \begin{block}{Why V-JEPA2-small?}
    ...
    pipeline can run on a \textbf{MacBook Pro} or any consumer laptop
    with sufficient GPU RAM — though embedding extraction remains slow
    on local hardware. For this project we used \textbf{cloud compute}
    to embed the full dataset in reasonable time.
  \end{block}
...
\begin{column}{0.52\paperwidth}
  ...
  \begin{block}{Classification Heads}
    \textbf{Relevance Classifier} — TBD
    ...
    The center token is fed to a single linear head predicting the
    \textbf{action} (a noun--verb pair).
    A 3-seed ensemble averages logits at inference.
